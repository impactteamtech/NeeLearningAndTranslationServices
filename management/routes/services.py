...
def update_service(
    ...
    if current_user.role != "admin" and db_service.tutor_id != current_user.id:
...
def delete_service(service_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    query_obj = db.get(Service, service_id)
    if not query_obj:
